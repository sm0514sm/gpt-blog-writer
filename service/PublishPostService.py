...
class PublishPostService:
  ...
  def publish_post(self, post: Post):
    ...
    write_page = Page(post.title, post.body, 3, category.id, post.tags)
    response = self.tistory_connector.write(write_page)

    post.status = "Published" if response.status_code == 200 else "Error"
    post.post_id = response.json()['tistory']['postId'] if response.status_code == 200 else ""
    ...
    topic.published_count += 1 if response.status_code == 200 else 0
